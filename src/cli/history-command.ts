...
export async function runDoctor(
  ...
  const human = `doctor status=${result.status}\n` + result.agents.map((agent) =>
    `${agent.agent}  ${agent.status}\n` +
    agent.locations.map((location) => `  ${location.role}  ${location.path}\n`).join("") +
    agent.findings.map((finding) => `  finding  ${finding}\n`).join("") +
    (agent.detail === undefined ? "" : `  detail  ${agent.detail}\n`)
  ).join("");
...
export async function runScan(
  ...
  if (result.status === "not_detected") {
    return success(
      ...
      "No supported Agent history source was detected.\n",
      globals.json,
    );
  }
  return success(
    ...
    `${result.agents.map((item) =>
      `Scanned ${item.agent}: ${item.sessions} session(s) · ${item.reusedSessions} reused · ` +
...
function renderConversationItem(item: ConversationItem): string {
  if (item.kind === "gap") {
    return `[gap${item.code === undefined ? "" : ` ${item.code}`}] ${item.label}\n`;
  }
...
export async function runHistory(globals: GlobalOptions, args: readonly string[]): Promise<CliResult> {
  ...
  if (action === "list") {
    ...
    const human = result.sessions
      .map((session) => `${session.sessionRef}  ${session.title || "(untitled)"}\n`)
      .join("");
    return success(
      "history list",
      {
      ...
      `${human}${renderPageSummary("session", result)}`,
      globals.json,
    );
  }
  if (action === "search") {
    ...
    const human = result.hits
      .map((hit) => `${hit.session.sessionRef}  ${hit.field}: ${hit.snippet}\n`)
      .join("");
    return success(
      "history search",
      {
      ...
      `${human}${renderPageSummary("hit", result)}`,
      globals.json,
    );
  }
  if (action === "show") {
    ...
    const body = session.conversation.map(renderConversationItem).join("");
    return success(
      "history show",
  ...
  if (
    ...
    return success(
      ...
      `${result.changed ? "Changed" : "No change"} ${result.sessionRef}: ${action} ` +
